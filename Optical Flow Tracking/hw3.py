...
def lucas_kanade(H, I):
    ...
    I_t = (I - H)
    sobel_x = np.array([[1, 0, -1], [2, 0, -2], [1, 0, -1]], dtype="float32") / 8.
    sobel_y = np.array([[1, 2, 1], [0, 0, 0], [-1, -2, -1]], dtype="float32") / 8.

    I_x = convolve_img(I, sobel_x)
    I_y = convolve_img(I, sobel_y)

    # Compute the various products (Ixx, Ixy, Iyy, Ixt, Iyt) necessary to form
    Ixx = np.sum((I_x * I_x) * mask)
    Ixy = np.sum((I_x * I_y) * mask)
    Iyy = np.sum((I_y * I_y) * mask)
    Ixt = np.sum((I_x * I_t) * mask)
    Iyt = np.sum((I_y * I_t) * mask)
...
def gaussian_pyramid(image, levels):
    ...
    for level in range(1, levels):
        c1 = convolve_img(image, gauss_kernel)
        decimate = 2 ** (level)
        pyr.append(c1[::decimate, ::decimate])
...
def track_object(frame1, frame2, boundingBox, steps):
    ...
    level = np.log2(min(w,h))
    levels = np.floor(level)
    levels = int(levels)
