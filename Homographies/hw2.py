...
def build_A(pts1, pts2):
    ...
    # pt1 is source poitns = x,y
    # pts2 is destination points = x', y' =. xhat and yhat
    # 9 unknown equation
    ...
    for i in range(0, pts2.shape[0]):
        x, y = pts1[i]
        xth, yth = pts2[i]
        A.append([x, y, 1, 0, 0, 0, -xth * x, -xth * y, -xth])
        A.append([0, 0, 0, x, y, 1, -yth * x, -yth * y, -yth])
...
def compute_H(pts1, pts2):
    ...
    smallest_eigenvalue = sorted_values[0]

    # TODO: Return the eigenvector corresponding to the smallest eigenvalue, reshaped
    # as a 3x3 matrix.
    eigenvector = eigenvectors[:, smallest_eigenvalue]
    eigenvector = eigenvector.reshape(3, 3)
...
def bilinear_interp(image, point):
    ...
    result = (1 - a) * (1 - b) * image[j][i] + a * (1 - b) * image[j][i + 1] + a * b * image[j + 1][i + 1] + (
            1 - a) * b * image[j + 1][i]
...
def apply_homography(H, points):
    ...
    # homogenous_points
    onesarray = np.ones((points.shape[0], 1))
    homogenous_coordinates = np.hstack((points, onesarray))

    # TODO: Apply the homography
    cartesian = []
    for value in homogenous_coordinates:
        homogenous = H.dot(value)
        x = homogenous[0]
        y = homogenous[1]
        z = homogenous[2]
        cartesian_values = [x / z, y / z]
        cartesian.append(cartesian_values)
...
def warp_homography(source, target_shape, Hinv):
    ...
    result = np.zeros(target_shape,dtype= sourcetype)
    ...
    for x in range(width):
        for y in range(height):
            ...
            # x,t location of source(ponts)
            #reshape to array of x and y, 1x2 
            # apply homography
            points = np.array([x, y])
            points = points.reshape(1,2)
            homoresult = apply_homography(Hinv,points)
            homoresult = np.squeeze(homoresult)
# TODO: check if the homography result is outside the source image. If so, move on to next pixel.
            
            i, j = homoresult

            if i > source.shape[1] or j > source.shape[0] or j <= 0 or i <= 0:
                continue
            else:
                    result[y][x] = bilinear_interp(source, homoresult) 
...
def rectify_image(image, source_points, target_points, crop):
    ...
    wraped_bounding_box = apply_homography(H ,bounding_box)

    sort_x = np.sort(wraped_bounding_box[:, 0])
    sort_y = np.sort(wraped_bounding_box[:, 1])
    ...
    rectified_bounding_box = apply_homography(T, wraped_bounding_box)

    # TODO: Compute the inverse homography that maps the rectified bounding box to the original bounding box
    invH = compute_H(rectified_bounding_box, bounding_box)
    # Determine the shape of the output image

    max_sort_x = np.sort(rectified_bounding_box[:, 0])
    max_sort_y = np.sort(rectified_bounding_box[:, 1])
...
# rectified bounding box
    # round up to int values 
    max_y = int(np.around(max_y))
    max_x = int(np.around(max_x))
    #height, width and channels
    shape = (max_y, max_x, 3)
# TODO: Finally call warp_homography to rectify the image and return the result
    rectifiedimage = warp_homography(image, shape, invH)
